...
public class DiaryPage_detail extends AppCompatActivity {
...
//    private String date;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_diary_page_detail);
...
//        imageUrl=getIntent().getStringExtra("imageUrl");
        ...
        buttonDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                datePickerDialog=new DatePickerDialog(DiaryPage_detail.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
        ...
        if(mImageUri != null){
            ...
            fileReference.putFile(mImageUri).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if(task.isSuccessful()){
                        Uri downloasUri=task.getResult();
                        Log.e("DiaryInsert","then:"+downloasUri.toString());

                        Upload upload=new Upload(textDate.getText().toString().trim()
                                ,editText_remarks.getText().toString().trim()
                                ,downloasUri.toString());

                        mDatabaseRef.push().setValue(upload);
                    }else {
                        Toast.makeText(DiaryPage_detail.this,task.getException().getMessage(),Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }else {
        ...
        switch (item.getItemId()) {
            case R.id.check:
                ...
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(DiaryPage_detail.this,"新增成功",Toast.LENGTH_SHORT).show();
                        Intent i = new Intent(DiaryPage_detail.this, DiaryPage.class);
                        startActivity(i);
                        finish();
                    }
                },3000);
